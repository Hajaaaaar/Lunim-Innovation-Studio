...
export default function CTASection({
  ...
  return (
    <div className="bg-gradient-to-b from-black to-gray-900 sm:py-32">
      <motion.div
      ...
      >
        <div className="bg-brand-light-dark/20 rounded-2xl grid grid-cols-1 lg:grid-cols-12 gap-12 items-center p-8 lg:p-12 border border-white/10">
            ...
            <div className="lg:col-span-7 text-center lg:text-left">
                ...
                <div className="mt-8">
                    <Link
                        href={buttonLink}
                        className="relative inline-block bg-gradient-to-r from-brand-blue to-brand-green text-black px-8 py-3 rounded-lg text-base font-bold transition-all duration-300 hover:scale-105 group"
                    >
                        <span className="absolute -inset-0.5 bg-gradient-to-r from-brand-blue to-brand-green rounded-lg blur opacity-75 group-hover:opacity-100 transition duration-1000 group-hover:duration-200 animate-pulse"></span>
            ...
            <div className="lg:col-span-5" ref={tiltRef}>
                <div className="bg-brand-light-dark/50 backdrop-blur-sm p-8 rounded-2xl shadow-xl border border-white/10 transform-style-3d group">
                    <h3 className="text-lg font-bold text-white">Contact Details</h3>
                    <p className="mt-4 text-gray-400">
                        Prefer to send a direct email?
                    </p>
                    <a href={`mailto:${email}`} className="mt-1 inline-block font-semibold text-brand-blue hover:text-brand-green transition-colors">
                        {email}
                    </a>
                        ...
                        <div className="mt-4 flex items-center gap-4">
                            {socialLinks.map(link => (
                                <a key={link.platform} href={link.url} className="group">
                                    <div className="w-10 h-10 bg-white/5 hover:bg-brand-green rounded-full flex items-center justify-center transition-colors duration-300">
                                        {icons[link.platform]}
                                    </div>
                                </a>
                            ))}
